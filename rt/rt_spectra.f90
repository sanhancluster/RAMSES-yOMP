...
!_________________________________________________________________________
  ...
  real(dp)::AGNSED_dlgM, AGNSED_dlgE
  real(dp)::AGNSED_lgM0, AGNSED_lgE0
  integer::AGNSED_nmass, AGNSED_nedd    ! Number of mass bins and edd bins
  real(dp),allocatable,dimension(:)::AGNSED_masses                ! [Msun]
  real(dp),allocatable,dimension(:)::AGNSED_edd
  ! AGNSED_table: iMasses, iEdd, igroups, properties
  !                                       (lumfrac, egy, csn, cse).
  real(dp),allocatable,dimension(:,:,:,:)::AGNSED_table
  ! ----------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  real(dp):: sum_LAGN(1:nSEDgroups)
  real(dp):: scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  real(dp):: scale_evtocode, scale_vol, scale_np, scale_fp, scale_msun
!-------------------------------------------------------------------------
  ...
  if (rt_star) then
     do i=1,npartmax
        ...
        do ip=1,nSEDgroups
           L_star(ip) = L_star(ip) * mass * scale_msun     !  [# photons s-1]
...
!-------------------------------------------------------------------------
  ...
  use amr_commons
  use rt_parameters
  integer ::  ilevel, icpu, ig, igrid, ip, ipart, jgrid, jpart, next_part, npart1, npart2
...
!-------------------------------------------------------------------------
  ...
  if(.not. same) then
     ...
     da0= min( max(   (age-SED_ages(ia)) /da,       0._dp ), 1._dp          )
     da1= min( max(  (SED_ages(ia+1)-age)/da,       0._dp ), 1._dp          )

     iz = min(max(floor((lgZ-SED_lgZ0)/SED_dlgZ ) + 1,   1  ),  SED_nZ-1 )
     dz = sed_Zeds(iz+1)-SED_Zeds(iz)
     dz0= min( max(   (Z-SED_zeds(iz)) /dz,         0._dp ),  1._dp         )
     dz1= min( max(  (SED_Zeds(iz+1)-Z)/dz,         0._dp ),  1._dp         )
...
!-------------------------------------------------------------------------
  use amr_commons
  use rt_parameters
  real(dp), intent(in):: mass, edd
  real(dp):: lgMass, lgEdd
  integer:: nProp
  logical:: same
  real(dp),dimension(:):: ret
  integer,save:: iM, iE
  real(dp),save:: dM, dM0, dM1, dE, dE0, dE1
!-------------------------------------------------------------------------
  if(.not. same) then
     if(Edd.gt.0) then
        lgEdd = log10(Edd)
...
!-------------------------------------------------------------------------
  use rt_parameters
  real(dp),intent(in):: Mbh_Msun, Edd_ratio
  real(dp),dimension(nSEDgroups):: ret
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  do j = 1, np
     ...
     dt_loc_Gyr = max(min(dt_Gyr, age), 0._dp)
